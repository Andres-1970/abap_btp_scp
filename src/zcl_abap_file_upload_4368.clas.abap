CLASS zcl_abap_file_upload_4368 DEFINITION
PUBLIC
FINAL
CREATE PUBLIC .
  PUBLIC SECTION.
...
CLASS zcl_abap_file_upload_4368 IMPLEMENTATION.
...
* extract the filename and fileext from this metadata as well as the raw file
*DATA.
        SPLIT request->get_text( ) AT cl_abap_char_utilities=>cr_lf INTO
      TABLE DATA(content).
        READ TABLE content REFERENCE INTO DATA(content_item) INDEX 2.
        IF sy-subrc = 0.
          SPLIT content_item->* AT ';' INTO TABLE DATA(content_dis).
          READ TABLE content_dis REFERENCE INTO DATA(content_dis_item) INDEX
          3.
          IF sy-subrc = 0.
...
* Check table name is valid.
        IF tablename IS INITIAL.
          response->set_status( i_code = if_web_http_status=>bad_request
                                i_reason = |Table name { tablename } notvalid or does not exist| ).
          response->set_text( |Table name { tablename } not valid or does notexist| ).
          RETURN.
        ENDIF.
...
| if (!oFileUploader.getValue()) \{ \n| && | sap.m.MessageToast.show("Choose a filefirst") \n| &&
| return \n| &&
| \} \n| &&
| let oInput = oCore.byId("tablename") \n| &&
| let oGroup = oCore.byId("grpDataOptions") \n| &&
| if (!oInput.getValue())\{ \n| && | sap.m.MessageToast.show("Target Table isRequired") \n| && | return \n| &&
| \} \n| &&
...
| input.addSuggestionItem(newsap.ui.core.Item(\{ \n| &&
